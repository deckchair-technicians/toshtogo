...
(with-redefs
  ...
  (facts "When requesting more work we may specify :or_existing_job. If a job exists with the same type and request_body as the requested dependency, this will be used instead of creating a new job"
         (let [job-id            (uuid)
               child-job-id      (uuid)
               parent-job-type   (uuid-str)
               child-job-type    (uuid-str)
